...
export const SORTS = ['newest', 'quality', 'popular', 'rarest', 'cheapest', 'context', 'set'] as const;
...
const COMPARE: Record<SortKey, (a: Model, b: Model) => number> = {
  ...
  // Unrated cards last; ties go to the newer card.
  quality: (a, b) => (b.quality?.value ?? -1) - (a.quality?.value ?? -1) || b.releaseDate.localeCompare(a.releaseDate),
