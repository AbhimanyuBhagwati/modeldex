...
import { getDataset, getLab, getModel, moreFromLab, rivalsOf } from '@/lib/data';
import {
  ACCESS_LABEL,
  INPUT_ONLY,
  RARITY_LABEL,
...
import { compareHref, modelHref } from '@/lib/site';
...
function licenseNote(m: Model, lab: LabSummary): string {
  switch (m.license.source) {
    ...
    case 'lab-default':
      return `No matching Hugging Face repo was found, so this shows ${lab.name}’s usual open-weight license. Check the model card before relying on it.`;
    default:
...
export default async function ModelPage({ params }: PageProps<'/models/[lab]/[id]'>) {
  ...
  const specs: [string, string][] = [
    ...
    ['Knowledge cutoff', formatMonth(m.knowledge)],
        ...
        <article className={styles.layout}>
          ...
          <div className={styles.info}>
            ...
            <div className={styles.actions}>
              ...
              {m.license.url && (
                <a className="btn" href={m.license.url} target="_blank" rel="noopener noreferrer">
            ...
            <p className={styles.note}>
              {licenseNote(m, lab)} Specs from models.dev, last changed {formatDate(data.updatedAt.slice(0, 10))}.
              {lab.docUrl && ` Official source: ${new URL(lab.docUrl).host}.`}
            </p>
          </div>
        </article>
